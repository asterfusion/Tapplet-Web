...
import { updateRoute } from './sf3000_updateRoute';
...
class SecurityLayout extends React.Component<SecurityLayoutProps, SecurityLayoutState> {
  ...
  componentDidMount() {
    ...
    if (dispatch) {
      dispatch({
        type: 'sf3000_login/fetchCurrent'
      })
...
export default connect(({ sf3000_login, sf3000_user, loading }: ConnectState) => ({
  status: sf3000_login.status,
  currentPerm: sf3000_user.currentPerm,
  currentRole: sf3000_user.currentRole,
  loading: loading.effects['sf3000_login/fetchCurrent'],
}))(SecurityLayout);
