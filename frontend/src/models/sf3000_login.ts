...
import { fetchCurrent, login, logout } from '@/services/sf3000_login';
...
const LoginMode1: Mode1Type = {
  ...
  effects: {
    ...
    *login({ payload }, { call, put }) {
      ...
      console.log("in login model", response)
      yield put({
    ...
    *logout(_, { call, put }) {
      ...
      if (window.location.pathname !== '/auth/login' && !redirect) {
        ...
        //yield document.cookie = 'str=;expires=Thu, 01-Jan-1970 00:00:01 GMT';
  ...
  reducers: {
    changeLoginStatus(state,action) {
      console.log("in login model reducer", action, action.payload)
      return {
        ...state,
