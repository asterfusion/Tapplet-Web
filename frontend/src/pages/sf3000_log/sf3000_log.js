...
@connect(({ sf3000_log, loading }) => {
  return {
    returnSysLogData: sf3000_log.returnIpReassemblyData,
    returnWaringLogData: sf3000_log.returnDeduplicationData,
  };
})
export default class SysLog extends React.Component {
  ...
  GMTToStr(time) {
    ...
    let search_day = s_day>9 ?s_day:('0'+s_day)
     let s_month =  date.getMonth()+1
    let search_month = s_month>9 ?s_month:('0'+s_month)
  ...
  render() {
    let { sortedInfo, filteredInfo, lang } = this.state
    if(localStorage.getItem("umi_locale") !== lang){
      this.fetchSysLog()
      this.setState({
        lang:localStorage.getItem("umi_locale")
      })
    ...
    const routes = [
      ...
      {
        path: 'log',
        breadcrumbName: formatMessage({id:"app.log.breadcrumb"}),
      },
      {
        path: '/log/log',
        breadcrumbName: formatMessage({id:"app.log.log.breadcrumb"}),
      }
    ];
    return (
      <PageHeaderWrapper
