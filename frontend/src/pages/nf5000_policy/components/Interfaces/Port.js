...
const Port = ({  blue, id, portName, isAddPort, isEditEGroup, portid, portConfig, canWrite, ...props }) => {
    ...
    const [{ isDragging }, drag] = useDrag({
        item: {name:portName, portnum: portid, type:ItemTypes.PORT},
        collect: monitor => ({
    ...
    //Styles
    const fill = blue ? '#5c7cb3' : '#fff'
    const opacity = isDragging ? 0.5 : 1
    const cursor = canWrite ? 'move' : 'default'
    return (
        <>
        <div ref={portRef} className={styles.in1_above}>
            <Button
                style={{
