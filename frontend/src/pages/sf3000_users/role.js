...
//import { updateRoute } from '@/layouts/sf3000_updateRoute';

const { pat_rolename } = RegexPattern
                ...
                <Form labelCol={{span: 5}} wrapperCol={{span: 15}}>
                                        ...
                                        if(value != undefined && value != ""){
                                            ...
                                            if(Mode == "Add"){
                                                ...
                                                if(role.indexOf(value) !== -1) {
                                                    throw new Error(formatMessage({id:"app.users.role.msg.rolename.format.duplication"})) 
                                                }
                                            }
                                            if(value != "" && value.length > 100){
                                                throw new Error(formatMessage({id:"app.users.role.msg.format.length"}))
                                            }
                                            if(value != "" && !pat_rolename.test(value)){
                    ...
                    <Form.Item label={formatMessage({id:"app.users.role.description"})}>
                        {getFieldDecorator("description",{
                            ...
                            rules:[{max: 100, message: formatMessage({id:"app.users.role.msg.description.format.length"})},]
...
class RolePermissionMenu extends Component {
    ...
    render() {
        ...
        const { newpermissions } = this.state
        let rolePerLabel = localStorage.getItem("umi_locale") == "zh-CN" ? {} : {width:"185px"}
        return (
...
@connect(({sf3000_user,loading})=>{
    return {
        roles: sf3000_user.roles,
        permissions: sf3000_user.permissions,
        userPerm: sf3000_user.currentPerm,
        roleCount: sf3000_user.roleCount
    }
})
export default class RolePage extends Component {
    ...
    // componentDidUpdate(prevProps){
    //     const { userPerm, route } = this.props
    //     console.log(route)
    //     if(!isEqual(prevProps.userPerm, userPerm)){
    //         console.log("1")
    //         let currentRole = localStorage.getItem("antd-pro-authority")
    //        // updateRoute(userPerm, currentRole, route)//怎么获取到全部的route???
    //     }
    // }
    //获取角色信息
    refreshRole = () => {
    ...
    refreshPermission = (role) => {
        const { dispatch } = this.props;
        dispatch({
            type: "sf3000_user/queryPermissions",
    ...
    render() {
        const columns = [
            {
                title: formatMessage({id:"app.users.role.rolename"}) ,
                dataIndex: 'rolename',
                key: 'rolename',
                sortDirections: ['descend']
            },
            {
                title: formatMessage({id:"app.users.role.description"}) ,
