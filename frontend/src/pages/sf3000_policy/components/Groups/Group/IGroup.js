...
const IGroup = React.forwardRef(
    ({itype, groupInfo, clickHandle, dltHandle, isCreateRuleGroup, deletePort, canWrite },ref) => {
        ...
        const dltDisabled = !canWrite || connect == true
        return (
            <div className={styles.group_wrapper} ref={igroupRef}>
                ...
                <Button className={styles.dltgroup} onClick={dltHandle} style={{opacity,backgroundColor:"white"}} disabled={dltDisabled}><Icon type="close" /></Button>
            </div>
        )
})
